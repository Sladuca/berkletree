...
impl<'params, const Q: usize> InternalNode<'params, Q> {
	fn new(params: &'params KZGParams<Bls12, Q>, key: Bytes, left: Node<'params, Q>, right: Node<'params, Q>) -> Self {
	...
	fn hash(&self) -> Result<FieldHash, BerkleError> {
		let commitment = self.prover.commitment_ref().ok_or(BerkleError::NotCommitted)?.inner();
